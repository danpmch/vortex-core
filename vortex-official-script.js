...
var App = (function () {
        ...
        $('#Vortexgear').on('click', '.tabBoxFunctionSet .lightbox .physicalKeyboard .key', function (e) {
            ...
            if (idx >= 0) {
                ...
                var filter = _this.vuePageData.data.functionSet.filter(function (functionSet) {
                    return functionSet.ktm.indexOf(fnktmCode) >= 0 && functionSet.profileIndex == _this.vuePageData.setting.profile;
                });
    ...
    App.prototype.downloadJson = function () {
        var blob = new Blob([JSON.stringify(this.vuePageData.data)], { type: "text/plain;charset=utf-8" });
        saveAs(blob, 'Vortex.json');
    };
    App.prototype.uploadJson = function (e) {
        var _self = this;
        var reader = new FileReader();
        reader.onload = function () {
            _self.vuePageData.data = JSON.parse(reader.result.toString());
        };
            ...
            $.each(filterFunctionSet, function (idx, row) {
                var profile = {
                    ...
                    key: {
                        FN: 0x94,
                        FN1: 0x95,
                        PN: _this.vuePageData.setting.keyboard == 'core4700' ? 0x96 : 0x95,
                        FN3: 0x97
                    }[row.key],
    ...
    App.prototype.convertToText = function () {
        ...
        text += '--- \r\n';
        text += 'Fn/pn change\r\n';
        this.vuePageData.data.functionSet.slice().sort(function (a, b) {
            return a.profileIndex > b.profileIndex ? 1 : -1;
        }).forEach(function (row) {
            text += " Layer " + row.profileIndex + "\r\n";
            text += '  ';
...
function getKeyClass(keybyte, fnKtm) {
    if (window['app']) {
        var vuePageData_1 = window['app'].vuePageData;
        return {
            'set': (vuePageData_1.active.tab == 'function_set' && vuePageData_1.active.data.ktm.indexOf(parseInt(fnKtm)) >= 0)
                || (vuePageData_1.tab == 'key_change' && findIdxFromObjArrayByKey(vuePageData_1.data.keyChange, {
                    sourceKey: keybyte,
                    sourceLayer: vuePageData_1.setting.layer,
                    profileIndex: vuePageData_1.setting.profile
                }) >= 0),
            'set_2': (vuePageData_1.tab == 'macro' && findIdxFromObjArrayByKey(vuePageData_1.data.macro, {
                sourceKey: keybyte,
                sourceLayer: vuePageData_1.setting.layer,
                profileIndex: vuePageData_1.setting.profile
            }) >= 0),
            'active': (vuePageData_1.active.tab == 'function_set' && vuePageData_1.data.functionSet.find(function (functionSet) { return functionSet.key !== vuePageData_1.active.data.key && functionSet.profileIndex == vuePageData_1.setting.profile && (functionSet.ktm.indexOf(parseInt(fnKtm)) >= 0); }))
                || (vuePageData_1.tab == 'macro' && findIdxFromObjArrayByKey(vuePageData_1.data.keyChange, {
                    sourceKey: keybyte,
                    sourceLayer: vuePageData_1.setting.layer,
                    profileIndex: vuePageData_1.setting.profile
                }) >= 0),
            'active_2': (vuePageData_1.tab == 'key_change' && findIdxFromObjArrayByKey(vuePageData_1.data.macro, {
                sourceKey: keybyte,
                sourceLayer: vuePageData_1.setting.layer,
                profileIndex: vuePageData_1.setting.profile
            }) >= 0),
            'preview': (vuePageData_1.active.tab == 'key_change' && vuePageData_1.active.data.targetKey != '0' && vuePageData_1.active.data.targetKey == keybyte)
        };
    }
    else {
        return {};
    ...
    switch (keyboard) {
            ...
            switch (language) {
                ...
                case 'de':
                    return [
                        ...
                        'EQUATION', 'BACKSPACE', 'R_BRACKETS', 'BACKSLASH', 'ENTER', 'CODE42', 'APP', '',
                        '', 'TILDE', '', '', 'R_WIN', '', '', 'R_CTRL',
                        '', '', '', '', '', '', '', 'CODE45',
                        'SPACE', '', 'L_WIN', 'L_CTRL', 'L_ALT', '', 'R_ALT', 'G10'
                    ];
            }
            break;
        case 'poker3':
            ...
            switch (language) {
                ...
                case 'nor':
                    return [
                        ...
                        'SPACE', 'CODE131', 'L_WIN', 'L_CTRL', 'L_ALT', 'CODE_132', 'R_ALT', 'R_WIN',
        ...
        case 'vtg8400':
            return [
                ...
                '8', '9', 'I', 'O', 'K', 'L', 'COMMA', 'DOT',
                '0', 'NEG', 'P', 'L_BRACKETS', 'SEMICOLON', 'APOSTROPHE', 'SLASH', 'R_SHIFT',
                'EQUATION', 'BACKSPACE', 'R_BRACKETS', 'BACKSLASH', 'ENTER', 'U_SP07', 'R_CTRL', 'U_SP01',
                'INSERT', 'HOME', 'DEL', 'END', 'G14', 'UP_ARROW', 'DN_ARROW', 'L_ARROW',
                'PGUP', 'U_SP00', 'PGDN', 'F10', 'F11', 'F12', 'R_ARROW', 'G15',
                'G10', 'U_SP06', 'L_WIN', 'L_CTRL', 'L_ALT', 'SPACE', 'R_ALT', 'G11',
                'ESC', 'F1', 'F2', 'F3', 'CODE45', 'CODE42', 'U_SP05', 'U_SP04',
                'F4', 'F5', 'F6', 'U_SP03', 'F7', 'F8', 'F9', 'U_SP02'
            ];
        default:
            return [];
    }
